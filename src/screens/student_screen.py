...
from src.components.dialog_browse_courses import browse_courses_dialog
from src.components.dialog_voice_update import voice_update_dialog
...
def student_dashboard():
    ...
    # ── Quick Enrollment via join link ───────────────────────────────────────
    ...
    # ── Voice sample warning banner ──────────────────────────────────────────
    has_voice = bool(student_data.get('voice_embedding'))
    if not has_voice:
        st.warning(
            "⚠️ **Voice sample not added yet.** "
            "Please add your voice sample so teachers can use voice attendance for you."
        )

    # ── Action row: enroll buttons + voice button ────────────────────────────
    ...
    with c2:
        ...
        with col_a:
            if st.button('📋 Browse Courses', type='primary',
                         width='stretch', key='browse_courses_btn'):
                browse_courses_dialog()
        with col_b:
            if st.button('🔑 Enter Code', width='stretch', key='enter_code_btn'):
                enroll_dialog()
    with c3:
        voice_label = "🎙️ Update Voice" if has_voice else "🎙️ Add Voice Sample"
        btn_type = "tertiary" if has_voice else "secondary"
        if st.button(voice_label, type=btn_type, width='stretch',
                     key='voice_update_btn'):
            voice_update_dialog()
    ...
    if not subjects:
        st.info("You are not enrolled in any subjects yet. "
                "Click **Browse Courses** above to join one!")
    else:
        cols = st.columns(2)
...
def student_screen():
    ...
    if show_registration:
        with st.container(border=True):
            ...
            st.info("Record a short voice clip for voice attendance. "
                    "You can also add or update this later from your dashboard.")
            ...
            try:
                ...
                st.error('Audio input not available.')
